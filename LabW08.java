...
public class LabW08 {
	...
	while(in.hasNextLine()) { 						//takes input until stop
	...
	for(int i = 0; i < input.length();i++) {	//goes through each letter
		if(input.charAt(i) == '+') {			//if letter is +
			String command = input.substring(start, i);	//get substring of large input string, only taking between the 2 +, so "+INSERT this+" is now "INSERT this"
			start = i+1; 						//increment starting point for next command
			if(command.split(" ")[0].equals("INSERT")) { //check command instruction
				q.i(command.split(" ")[1]);				//if insert split string to insert the word
			} else if(command.split(" ")[0].equals("REMOVE")) {//check command instruction
				q.r();										//remove front item from queue
			}
		}
	}
...
class Queue{
	...
	public String get() {
		...
		//System.out.println(front + " " + rear + " " + nItems);
